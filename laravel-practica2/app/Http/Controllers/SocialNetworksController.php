<?php

namespace App\Http\Controllers;

use App\Models\SocialNetworks;
use App\Http\Requests\StoreSocialNetworksRequest;
use App\Http\Requests\UpdateSocialNetworksRequest;

class SocialNetworksController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreSocialNetworksRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(SocialNetworks $socialNetworks)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(SocialNetworks $socialNetworks)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateSocialNetworksRequest $request, SocialNetworks $socialNetworks)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(SocialNetworks $socialNetworks)
    {
        //
    }
}
